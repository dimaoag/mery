...
class MainController extends AppController
{
    ...
    public function indexAction(){
        ...
        $articles = \R::findAll( 'article', 'WHERE status = 1 AND is_show = 1 LIMIT 3' );
        $videos = \R::findAll( 'video', 'WHERE is_show = 1 AND status = 1 LIMIT 3' );
        $reviews = \R::getAll("SELECT review.*, user.first_name, user.last_name, user.photo_origin, user.photo_profile FROM review JOIN user ON review.user_id = user.id WHERE review.status = 1 LIMIT 7");
